...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables? - They are joined by a linking table, meaning there is a third table that holds the columns required for them to interact. These tables otherwise have nothing in common, which is why a linking table is made.
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate? - A document-oriented database like MongoDB is best for situations where you are dealing with large amounts of rapidly-changing data that don't need a lot of upfront structure. However, in situations where you are developing a product that requires long-term stability and reliability, they would not be as useful or helpful.
- What is "NewSQL", and what is it trying to achieve? - NewSQL is trying to be the "best of both worlds" in database management. It tries to accomplish the scalability of NoSQL (to accommodate for Big Data), while maintaining consistency, durability, unit-wide functionality.
